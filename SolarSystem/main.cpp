...
BOOL CreateGLWindow(char* title, int width, int height, int bits)
{
    ...
    0, wHeight, wWidth, 100, hWnd, NULL, hInstance, NULL );*/
...
LRESULT CALLBACK WndProc(	HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM	lParam)
{
	...
	switch (uMsg)
	{
	  ...
	  break;
		...
		case WM_MOUSEWHEEL: {
			int rolldir = GET_WHEEL_DELTA_WPARAM(wParam);
			SC->MouseRolled(rolldir);
			SC->Draw();
			SwapBuffers(hDC);
			return 0;
		}
		 ...
		 case WM_SIZE:      // zmiana rozmiaru okna 
		{
			...
			MoveWindow(hListBox,0,wHeight,wWidth,0,true);
